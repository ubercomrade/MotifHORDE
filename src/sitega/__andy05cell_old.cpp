...
#include <ctype.h>
#include <stdint.h>
#include <algorithm>  // std::sort (replaces qsort)
...
#define MEGE 500//population size max (compile-time)
#define MEGE_DEFAULT 100//default population size
#define NMUT 3
#define NREC 6
#define POPSIZE 120
//#define GA_EXIT 0.01

// --- Stage 2: Seeded deterministic RNG (xorshift64*) ---
struct RNG {
  uint64_t s;
  explicit RNG(uint64_t seed = 0) : s(seed ? seed : 0x9E3779B97F4A7C15ULL) {}
  inline void seed(uint64_t v) { s = v ? v : 0x9E3779B97F4A7C15ULL; }
  inline uint32_t operator()() {
    s ^= s >> 12; s ^= s << 25; s ^= s >> 27;
    return (uint32_t)((s * 0x2545F4914F6CDD1DULL) >> 32);
  }
  inline int range(int n) { return (int)((*this)() % (uint32_t)n); }
};
static thread_local RNG g_rng;
// --- End Stage 2 ---

// --- Stage 6: thread-local globals for OpenMP ---
static thread_local double uw[POPSIZE][POPSIZE];
struct ss {
	int num;
	char oli[3];
}s[16];
...
// Comparators for std::sort (replacing qsort)
static inline bool cmp_tot(const uno& a, const uno& b) {
	if (a.num != b.num) return a.num < b.num;
	return a.sta < b.sta;
}
static inline bool cmp_pop(const town& a, const town& b) {
	return a.fit > b.fit;
}
...
void BigMixI(int* d1, int len) // pereme6ivanie stroki
{
	...
	for (r = 0; r < len - 1; r++)
	{
		MixI(&d1[r], &d1[1 + r + (g_rng.range(len - 1 - r))]);
	}
}
...
void BigMixC(char* d1, int len) // pereme6ivanie stroki
{
	...
	for (r = 0; r < len - 1; r++)
	{
		MixC(&d1[r], &d1[1 + r + (g_rng.range(len - 1 - r))]);
	}
}
...
void town::get_copy(town* a, int nseq, int reg_max)
{
	...
	a->fpr = fpr;
	a->inf = inf;
	memcpy(a->tot, tot, size * sizeof(uno));
	memcpy(a->deg, deg, 16 * sizeof(int));
	memcpy(a->odg, odg, reg_max * sizeof(int));
	memcpy(a->pos, pos, nseq * sizeof(int));
	memcpy(a->ori, ori, nseq * sizeof(int));
}
...
double EvalMahFIT(town* a, int n_train, int octa, int infc, int*** seq, int olen, double** dav, double** dcv, double** octa_prow, double* pexp, int* len, FILE* outlog)//double *hoxa_wei, qbs *qps, double **frp, double *octa_rat,
{
	...
	for (k = 0; k < a->size; k++) memset(uw[k], 0, a->size * sizeof(double));
	...
	for (m = 0; m < n_train; m++)
	{
		...
		// Stage 3: sparse covariance — only iterate over non-zero fs indices
		int nz[POPSIZE], nnz = 0;
		for (k = 0; k < a->size; k++) if (fs[k] != 0) nz[nnz++] = k;
		for (int p = 0; p < nnz; p++)
		{
			int k = nz[p];
			for (int q = 0; q <= p; q++) // upper triangle only
				uw[k][nz[q]] += fs[k] * fs[nz[q]];
			f1[k] += fs[k];
		}
	}
	// Stage 3: mirror lower triangle to upper triangle (symmetry)
	// Accumulation filled lower triangle (row >= col), copy to upper
	for (k = 0; k < a->size; k++)
		for (n = 0; n < k; n++)
			uw[n][k] = uw[k][n];
	for (k = 0; k < olen1; k++)
...
int MutCry0(town* a, int oln, int reg_max)
{
	...
	int w = g_rng.range(ab);
	int loc = b1 - a1 - w + 1;
	if (loc <= 0)return -1;
	int da2 = g_rng.range(loc);
	a2 = a1 + da2;
	b2 = a2 + w;
...
int RecFeat(town a1, town a2, int(*cop)[2], int max)
{
	...
	do
	{
		...
		{
			gom2 = 0;
			if (gom1 == 0)r2 = g_rng.range(a2.size - ret);
			else r2 = r0;
			for (i = 0; i < ret; i++)
			{
				if (r2 >= cop[i][1])r2++;
...
int Reco2_Original(town* a1, town* a2, int* nsi, int* num)
{
	...
	{
		gom2 = 0;
		if (gom1 == 0)r2 = g_rng.range(a2->size);
		else r2 = r0;
		num2 = a2->tot[r2].num;
		if (a1->deg[num2] == 0)break;
		if (num1 == num2 && (a1->tot[r1].end == a2->tot[r2].end && a1->tot[r1].sta == a2->tot[r2].sta))continue;
...
int GomTown(const town& a, const town& b, int nseq, int check_lpd)
{
	int i;
	for (i = 0; i < nseq; i++)
	{
...
int GomTown2(const town& a, const town& b)
{
	int i;
	for (i = 0; i < a.size; i++)
	{
...
int sitega_train(const TrainParams* p, TrainResult* r)
{
	...
	// Stage 4: verbose flag
	int verbose = p->verbose;

	// Stage 5: read params from struct instead of argv
	strcpy(path_fasta, p->fg_path);
	strcpy(file_for, p->fg_file);
	strcpy(file_back, p->bg_file);
	strcpy(pfile_for, path_fasta);
	strcpy(pfile_back, p->bg_path ? p->bg_path : path_fasta);
	strcat(pfile_for, file_for);
	strcat(pfile_back, file_back);
	int reg_max = p->max_lpd;// max LPD length
	// Population size from parameter (default MEGE_DEFAULT, max MEGE)
	int mege_h = (p->pop_size > 0 && p->pop_size <= MEGE) ? p->pop_size : MEGE_DEFAULT;
	int olen = p->motif_len;// dlina motiva
	int size = p->size;
	int octa = p->olig_bg;
	int infc = p->infc; // information content in GA fitness
	strcpy(path_out, p->out_path);
	int len_peak_max = p->max_peak_len; //2500;
	strcpy(file_log, path_out);
	strcat(file_log, p->log_file);
	...
	int olen1 = olen - 1;
	// Stage 2: seeded RNG
	unsigned long seed_val = p->seed ? p->seed : (unsigned long)time(NULL);
	g_rng.seed((uint64_t)seed_val);
	fprintf(outlog, "RNG seed: %lu\n", seed_val);
	if (size > POPSIZE)
	{
	...
	for (i = 0; i < 2; i++)
	{
		...
		for (j = 0; j < nseq; j++)
		{
			...
			if (seq_real[i][j] == NULL) { puts("Out of memory..."); return 1; }
		}
	}
	for (i = 0; i < 2; i++)
	{
	...
	if (octa_pro1p == NULL) { puts("Out of memory..."); return 1; }
	fprintf(outlog, "Start1\n");
	double* octa_rat;
	octa_rat = new double[octa_size];
	if (octa_rat == NULL) { puts("Out of memory..."); return 1; }
	double* octa_av;
	octa_av = new double[octa_size];
	if (octa_av == NULL) { puts("Out of memory..."); return 1; }
	int* octa1;
	octa1 = new int[octa_size];
	if (octa1 == NULL) { puts("Out of memory..."); return 1; }
	...
	det1.mem_in(nseq);
	qp = new double[nseq];
	if (qp == NULL) { puts("Out of memory..."); return 1; }
	for (i = 0; i < nseq; i++)qp[i] = 0;
	thr_octa = new double[nseq];
	if (thr_octa == NULL) { puts("Out of memory..."); return 1; }
	len_octa = new int[nseq];
	if (len_octa == NULL) { puts("Out of memory..."); return 1; }
	...
	for (i = 0; i < nseq; i++)
	{
		...
		int half = leni / 3 - 1;
		{
			...
			std::sort(octa_pro1p, octa_pro1p + leni, std::greater<double>());
	...
	double jwei0[3];
	{
		//= { 1.02, 1.05, 1.1 };
		int rwei[MEGE];
		double jwei1 = pow(double(20), double(2) / (mege_h + 2));//= jwei0[2];
		jwei0[2] = jwei1;
		jwei0[1] = 1 + (jwei1 - 1) / 2;
		jwei0[0] = 1 + (jwei1 - 1) / 5;
	...
	for (i = 0; i < mege_h; i++)stop_oi[i] = stop_li[i] = stop_pi[i] = 0;
	...
	do
	{
		...
		if (big_exit1 == 1)
		{
			...
			big_exit1 = 0;
			std::sort(pop, pop + mege_h, cmp_pop);
		...
		int* success_l_l = new int[nthreads];
		int* success_p_l = new int[nthreads];
		int* success_m_l = new int[nthreads];
		int* success_m_tot_l = new int[nthreads];
		int* n_mut_tot_l = new int[nthreads];
		for (int t = 0; t < nthreads; t++) {
			for (k = 0; k < NMUT; k++) atry_l[t][k] = asuccess_l[t][k] = 0;
			success_o_l[t] = success_l_l[t] = success_p_l[t] = 0;
			success_m_l[t] = success_m_tot_l[t] = n_mut_tot_l[t] = 0;
		}
		do
		{
			...
			for (i = 0; i < mege_h; i++)
			{
				if (stop_pi[i] == 0 || (stop_li[i] == 0 || stop_oi[i] == 0))
				{
					...
					while (ratio[2] > ratio_thr || (ratio[0] > ratio_thr || ratio[1] > ratio_thr))
					{
						...
						if (n_mut_here % step == 0)
						{
							...
							for (k = 0; k < 3; k++)atry_l[tid][k] += step_try[k];
							for (k = 0; k < 3; k++)asuccess_l[tid][k] += step_success[k];
					...
					success_o_l[tid] += success_o_local;
					success_l_l[tid] += success_l_local;
					success_p_l[tid] += success_p_local;
				...
				//	if ((gen == 0 && i == 0) || gen > 0)
					...
					n_mut_tot_l[tid] += n_mut_here;
					success_m_l[tid] += (success_m_local != 0) ? 1 : 0;
				}
			}
			// Reduce per-thread accumulators into global
			for (int t = 0; t < nthreads; t++) {
				for (k = 0; k < NMUT; k++) {
					atry[k] += atry_l[t][k];
					asuccess[k] += asuccess_l[t][k];
					atry_l[t][k] = asuccess_l[t][k] = 0;
				}
				success_o += success_o_l[t];
				success_l += success_l_l[t];
				success_p += success_p_l[t];
				success_m += success_m_l[t];
				success_m_tot += success_m_tot_l[t];
				n_mut_tot += n_mut_tot_l[t];
				success_o_l[t] = success_l_l[t] = success_p_l[t] = 0;
				success_m_l[t] = success_m_tot_l[t] = n_mut_tot_l[t] = 0;
			}
			mut_jump = 0;
		...
		do
		{
			...
			if (sr % sr_step_cycle == 0)
			{
				...
				tnow = time(NULL);
				if (verbose) fprintf(outlog, "%s", ctime(&tnow));
				for (k = 0; k < mege_h; k++)rec_jump[k] = 0;
				if (loc_rec == 0)break;
	...
	big_exit1 = 1;
	// Stage 5: fill TrainResult with output paths and best fitness
	if (r)
	{
		sprintf(r->mat_path, "%s_mat1", file_base);
		sprintf(r->loc_path, "%s_loc1", file_base);
		r->best_fit = pop[0].fit;
		r->status = 0;
	}
	fclose(outlog);
	for (i = 0; i < mege_h; i++)pop[i].mem_out();
...
// Stage 5: thin CLI wrapper — parses argv into TrainParams and calls sitega_train
int main(int argc, char* argv[])
{
	if (argc != 12 && argc != 13 && argc != 14)
	{
	puts("Sintax: 1path 2fg_fasta 3bg_fasta 4max_LPD 5motif_len 6size 7olig_bg 8infc 9out_path 10max_peak_len 11log [12seed] [13pop_size]");
		return 1;
	}
	TrainParams p;
	memset(&p, 0, sizeof(p));
	...
	p.fg_file = argv[2];
	p.bg_file = argv[3];
	p.max_lpd = atoi(argv[4]);
	p.motif_len = atoi(argv[5]);
	p.size = atoi(argv[6]);
	p.olig_bg = atoi(argv[7]);
	p.infc = atoi(argv[8]);
	p.out_path = argv[9];
	p.max_peak_len = atoi(argv[10]);
	p.log_file = argv[11];
	p.seed = (argc >= 13) ? strtoul(argv[12], NULL, 0) : 0;
	p.pop_size = (argc >= 14) ? atoi(argv[13]) : 0;
	p.verbose = (getenv("SITEGA_VERBOSE") != NULL) ? atoi(getenv("SITEGA_VERBOSE")) : 0;
