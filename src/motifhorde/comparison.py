...
class TomtomComparator(GeneralMotifComparator):
    ...
    def compare(self, motifs_1, motifs_2, sequences=None):
        ...
        for motif in motifs_1:
            records.extend(compare_one_to_many(motif, motifs_2, "motif", self.config, sequences=sequences))
...
class UniversalMotifComparator(GeneralMotifComparator):
    ...
    def __init__(
        ...
        super().__init__(name)
        metric = "cosine" if metric == "corr" else metric
    ...
    def compare(self, motifs_1, motifs_2, sequences=None):
        records = []
        for motif in motifs_1:
            records.extend(compare_one_to_many(motif, motifs_2, "profile", self.config, sequences=sequences))
        frame = _records_to_frame(records)
        if self.filter_type is None or frame.empty:
            return frame
        if self.filter_type not in frame.columns:
            raise ValueError(f"Comparison results do not contain filter column: {self.filter_type}")
        if self.filter_type == "p-value":
            return frame[frame[self.filter_type] <= self.filter_threshold].reset_index(drop=True)
        return frame[frame[self.filter_type] >= self.filter_threshold].reset_index(drop=True)
