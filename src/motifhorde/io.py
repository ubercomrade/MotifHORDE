...
def write_fasta(sequences, path: str | Path) -> None:
    ...
    if isinstance(sequences, dict) and {"values", "lengths"}.issubset(sequences):
        rows = (row_values(sequences, index) for index in range(len(sequences["lengths"])))
    else:
...
def write_meme(motifs: List[np.ndarray], info: List[Tuple[str, int]], path: str | Path) -> None:
    ...
    with open(path, "w") as handle:
        ...
        for pfm, (name, length) in zip(motifs, info, strict=True):
            ...
            handle.write(f"letter-probability matrix: alength= 4 w= {int(length)} nsites= 20 E= 0\n")
...
def _meme_length_from_header(header_line: str) -> int:
    match = _MEME_WIDTH_RE.search(header_line)
    if match is None:
        raise ValueError(f"Malformed MEME matrix header: {header_line.strip()}")
...
def read_bamm(motif_path: str, target_order: int) -> np.ndarray:
    ...
    if target_order > max_order_file:
        requested_order = target_order
        target_order = max_order_file
        logger = logging.getLogger(__name__)
        logger.warning(
            "Target order %s exceeds file max order %s, target order set as max order",
            requested_order,
            max_order_file,
        )
...
def _parse_dimont_treeelement(elem: ET.Element) -> dict:
    ...
    if pars_pos:
        ...
        for pos in pars_pos:
            par = pos.find("parameter")
            symbol = int(_xml_numeric_value(par.find("symbol")))
            scores[symbol] = _xml_numeric_value(par.find("value"))
    ...
    for pos in children_elem:
        ...
        children[child_index] = _parse_dimont_treeelement(pos.find("treeElement"))
