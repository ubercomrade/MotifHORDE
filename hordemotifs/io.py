...
from .functions import pfm_to_pwm



def read_fasta(path: str, return_ragged: bool = True) -> Union[RaggedData, List[np.ndarray]]:
    ...
    for char, code in zip(b"ACGTacgt", [0, 1, 2, 3] * 2):
...
def read_meme(path: str, index: int = 0) -> Tuple[np.ndarray, Tuple[str, int], int]:
    ...
    """
    target_motif: np.ndarray = None
    target_info: Tuple[str, int] = None
    motif_count = 0
...
def write_meme(motifs: List[np.ndarray], info: List[Tuple[str, int]], path: str) -> None:
    ...
    with open(path, "w") as out:
        ...
        for motif, (name, length) in zip(motifs, info):
...
def read_bamm(motif_path: str, bg_path: str, target_order: int) -> np.ndarray:
    ...
    bg_raw, _, _ = parse_file_content(bg_path)

    if target_order > max_order_file:
        raise ValueError(f"Target order {target_order} exceeds file max order {max_order_file}")
