...
from .functions import (
                        ...
                        batch_all_scores,
                        batch_best_scores,
                        batch_get_frequencies,
                        batch_get_hit_counts,
                        pfm_to_pwm)
from .ragged import RaggedData
from .io import read_sitega, read_meme, read_bamm, read_pfm
...
class MotifModel:
    ...
    def threshold_table(self) -> np.ndarray:
        ...
        if self._threshold_table is None:
            print(f"There is no threshold table for model: {self.name}")
    ...
    def get_threshold_table(self, promoters) -> np.ndarray:
        ...
        """
        print(f"Computing threshold table for model: {self.name}")
    ...
    def scan(self, sequences: RaggedData, strand: Optional[StrandMode] = None) -> RaggedData:
        ...
        strand : {"+", "-", "both", "best"}, optional
            Strand selection mode:
            - None: use self.strand_mode.
            - "+": forward strand only.
            - "-": reverse complement only.
            - "both": both strands (returns RaggedData with 2*N sequences).
    ...
    @classmethod
    def from_file(cls, path: str) -> 'MotifModel':
        """
    ...
    def get_sites(
        ...
        score_threshold = self._frequency_to_score(fpr_threshold) if mode == "threshold" else None
        if score_threshold is not None:
            print(f"FPR threshold: {fpr_threshold} → Score threshold: {score_threshold:.4f}")
        ...
        # Batch scanning
        scans = self.scan(sequences, strand="both")
        n_seq = sequences.num_sequences

        for seq_idx in range(n_seq):
            seq = sequences.get_slice(seq_idx)
            s_fwd = scans.get_slice(seq_idx)
            s_rev = scans.get_slice(seq_idx + n_seq)
        ...
        print(f"Found {len(df)} site(s) in {sequences.num_sequences} sequence(s)")
        return df
    ...
    def write_dist(self, path: str) -> None:
        ...
        path : str
            Path of the output file.
        """
        table = copy.deepcopy(self.threshold_table)
    ...
    def pfm(self) -> Optional[np.ndarray]:
        ...
        if self._pfm is None:
            print(f"PFM not computed for model: {self.name}. Use get_pfm(sequences) to compute.")
        return self._pfm
    ...
    def get_pfm(
        ...
        if self._pfm is not None and not force_recompute:
            print(f"Returning cached PFM for model: {self.name}")
            return self._pfm

        print(f"Computing PFM for model: {self.name}")
        ...
        if top_fraction is not None:
            ...
            print(f"Selected top {top_fraction*100:.1f}%: {n_keep} sites")
    ...
    def get_frequencies(self, sequences: RaggedData, strand: Optional[StrandMode] = None) -> RaggedData:
        ...
        """
        strand = self.strand_mode if strand is None else strand
        matrix = self.matrix.astype(np.float32)
        kmer = getattr(self, 'kmer', 1)

        if strand == "+":
            return batch_get_frequencies(sequences, matrix, kmer=kmer, is_revcomp=False)
        elif strand == "-":
            return batch_get_frequencies(sequences, matrix, kmer=kmer, is_revcomp=True)
        elif strand == "best":
            # For "best" frequencies, we take maximum of frequencies from both strands
            ff = batch_get_frequencies(sequences, matrix, kmer=kmer, is_revcomp=False)
            rf = batch_get_frequencies(sequences, matrix, kmer=kmer, is_revcomp=True)
            return RaggedData(np.maximum(ff.data, rf.data), ff.offsets)
        elif strand == "both":
            ff = batch_get_frequencies(sequences, matrix, kmer=kmer, is_revcomp=False)
            rf = batch_get_frequencies(sequences, matrix, kmer=kmer, is_revcomp=True)
            new_data = np.concatenate([ff.data, rf.data])
            offsets_r = rf.offsets[1:] - rf.offsets[0]
            new_offsets = np.concatenate([ff.offsets, ff.offsets[-1] + offsets_r])
            return RaggedData(new_data, new_offsets)
        
        raise ValueError(f"Unknown strand mode: {strand}")

    def get_hit_counts(self, sequences: RaggedData, threshold: float, strand: Optional[StrandMode] = None) -> np.ndarray:
        """Calculate site hit counts per sequence using batch processing.
        ...
        sequences : RaggedData
            Encoded nucleotide sequences.
        threshold : float
            Score threshold for considering a site a match.
        strand : {"best", "+", "-", "both"}, optional
            Strand to evaluate.
        ...
        -------
        np.ndarray
            Number of matches (hits) above threshold for each sequence.
        """
        strand = self.strand_mode if strand is None else strand
        ...
        is_revcomp = (strand == "-")
        both_strands = (strand == "best" or strand == "both")

        return batch_get_hit_counts(sequences, matrix, threshold, kmer=kmer, is_revcomp=is_revcomp, both_strands=both_strands)
    ...
    def load(filepath: str) -> 'MotifModel':
        ...
        WARNING: Loading files from untrusted sources can result in arbitrary
        code execution. Only load files you trust.
        ...
        print(f"WARNING: Loading model from {filepath}. Only load trusted files to prevent security risks.")
...
class PwmMotif(MotifModel):
    ...
    scoring using the precompiled :func:`hordemotifs.functions.all_scores`
    ...
    def scan(self, sequences: RaggedData, strand: Optional[StrandMode] = None) -> RaggedData:
        ...
        if strand == "both":
            sf = batch_all_scores(sequences, matrix, kmer=self.kmer, is_revcomp=False)
            sr = batch_all_scores(sequences, matrix, kmer=self.kmer, is_revcomp=True)
            new_data = np.concatenate([sf.data, sr.data])
            offsets_r = sr.offsets[1:] - sr.offsets[0]
            new_offsets = np.concatenate([sf.offsets, sf.offsets[-1] + offsets_r])
            return RaggedData(new_data, new_offsets)
        raise ValueError(f"Unknown strand mode: {strand}")

    @classmethod
    def from_file(cls, path: str, index: int = 0) -> PwmMotif:
        ...
        if ext == '.meme':
            # MEME format
            matrices, info, _number_of_motifs = read_meme(path, index=index)
            if len(matrices) == 0:
                raise ValueError(f"No motifs found in MEME file: {path}")
            pfm = matrices[0]
            name = info[0][0] if info else os.path.splitext(os.path.basename(path))[0]
            length = info[0][1] if info else pfm.shape[1]
        elif ext == '.pfm':
            # PFM format
            ...
            # Default to PFM format
            pwm, length, minimum, maximum = read_pfm(path)
            pfm = pwm[:4, :]  # Get the original PFM from the extended PWM
            name = os.path.splitext(os.path.basename(path))[0]
...
class BammMotif(MotifModel):
    ...
    @classmethod
    def from_file(cls, path: str, bg_path: str = None, order: int = 2) -> 'BammMotif':
        """Create a BammMotif from a file.
        
        Parameters
        ----------
        path : str
            Path to the BaMM motif file (.ihbcp format).
        ...
        length = matrix.shape[-1]  # Length is the last dimension
        name = os.path.splitext(os.path.basename(path))[0]
...
class SitegaMotif(MotifModel):
    ...
    @classmethod
    def from_file(cls, path: str) -> SitegaMotif:
        ...
        matrix, length, _minimum, _maximum = read_sitega(path)
